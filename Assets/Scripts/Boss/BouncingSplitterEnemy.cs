...
public class BouncingSplitterEnemy : BossEnemy
{
    ...
    protected override IEnumerator Start()
    {
        ...
        if(_isFirst)
        {
            _direction = new Vector3(0.66f, 0.66f, 0).normalized;
        }
        _movementSpeed = 2;
        while (_isAlive)
        {
    ...
    protected IEnumerator SplitCoroutine()
    {
        ...
        if(SplitCount < 4)
        {
            ...
            newObj2.Initialize((_direction + Random.Range(0.0f, 0.16f) * new Vector3(1f, 1f, 0f)).normalized, _movementSpeed, _damage);
